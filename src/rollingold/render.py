...
<body>
        ...
        <div class="section-head">
          ...
          <div class="chart-meta">
            ...
            <div class="trace-switch" aria-label="轨迹范围切换">
              <button data-trace-mode="20" type="button">20日</button>
              <button class="active" data-trace-mode="60" type="button">60日</button>
              <button data-trace-mode="latest" type="button">仅点位</button>
            </div>
            <span>横轴看相对位置，纵轴看变化速度</span>
          </div>
        </div>
        <div class="rotation-explain" aria-label="轮动图图例说明">
          <p><span>横轴：相对强弱</span>越靠右，行业相对申万 A 指越强；越靠左，表示相对跑输。</p>
          <p><span>纵轴：相对动量</span>越靠上，相对强弱改善越快；越靠下，表示相对动能转弱。</p>
          <p><span>轨迹方向</span>线段越深越接近当前，末端圆点是最新位置；成交占比只作为圆点大小的辅助提示。</p>
        </div>
  ...
  <script>
    ...
    let hovered = null;
    let traceMode = initialParams.get('trace') || storedState.trace || '60';
    ...
    function chartLimit(points) {{
      const values = [];
      points.forEach(entry => values.push(Math.abs(entry.point.x), Math.abs(entry.point.y)));
      if (traceMode !== 'latest') {{
        DATA.industries.forEach(item => {{
          displayPath(activePoint(item).path).forEach(point => values.push(Math.abs(point.x), Math.abs(point.y)));
        }});
      }}
      const raw = Math.max(1.2, ...values.filter(Number.isFinite));
      return Math.max(4, Math.ceil((raw + 0.2) * 2) / 2);
    }}
    ...
    function arrowHead(points, limit, color, options = {{}}) {{
      if (!points || points.length < 2) return '';
      const last = points[points.length - 1];
      const previous = points[points.length - 2];
      const x = scaleX(last.x, limit);
      const y = scaleY(last.y, limit);
      const prevX = scaleX(previous.x, limit);
      const prevY = scaleY(previous.y, limit);
      const angle = Math.atan2(y - prevY, x - prevX);
      const size = options.size || 5.5;
      const spread = .72;
      const p1 = `${{x.toFixed(1)}},${{y.toFixed(1)}}`;
      const p2 = `${{(x - Math.cos(angle - spread) * size).toFixed(1)}},${{(y - Math.sin(angle - spread) * size).toFixed(1)}}`;
      const p3 = `${{(x - Math.cos(angle + spread) * size).toFixed(1)}},${{(y - Math.sin(angle + spread) * size).toFixed(1)}}`;
      return `<polygon points="${{p1}} ${{p2}} ${{p3}}" fill="${{color}}" opacity="${{options.opacity ?? .58}}"></polygon>`;
    }}
    function labelNames() {{
      const names = new Set();
      [
        ...(DATA.rankings?.top_strength || []).slice(0, 5),
        ...(DATA.rankings?.weak_repair || []).slice(0, 2),
        ...(DATA.rankings?.slowing || []).slice(0, 2),
        ...(DATA.rankings?.deteriorating || []).slice(0, 2)
    ...
    function renderRotation() {{
      ...
      const limit = chartLimit(points);
      const ticks = [-limit, -limit / 2, 0, limit / 2, limit];
      const labels = [
        ['走强', 214, 134], ['领涨', 506, 134], ['领跌', 214, 286], ['走弱', 506, 286]
      ];
      const visibleLabels = labelNames();
      const pathEntries = DATA.industries.map(item => {{
        const point = activePoint(item);
        const path = displayPath(point.path);
        const isFocused = item.name === focusName;
        const isSelected = item.name === selected;
        return {{ item, point, path, isFocused, isSelected }};
      }}).filter(entry => entry.path.length >= 2);
      const quietPaths = pathEntries.filter(entry => !entry.isFocused && !entry.isSelected);
      const focusPaths = pathEntries.filter(entry => entry.isFocused || entry.isSelected);
      svg.innerHTML = `
        ...
        ${{quietPaths.map(entry => {{
          const color = colorMap[entry.point.quadrant] || '#66716d';
          return lineSegments(entry.path, limit, {{ startColor: '#dde6e1', endColor: color, opacityStart: .08, opacityEnd: hovered ? .18 : .34, widthStart: .9, widthEnd: 1.45 }}) +
            arrowHead(entry.path, limit, color, {{ size: 4.6, opacity: hovered ? .14 : .30 }});
        }}).join('')}}
        ${{focusPaths.map(entry => {{
          const color = colorMap[entry.point.quadrant] || '#66716d';
          return lineSegments(entry.path, limit, {{ startColor: '#b5c5bd', endColor: color, opacityStart: .44, opacityEnd: .92, widthStart: 1.55, widthEnd: 3.05 }}) +
            arrowHead(entry.path, limit, color, {{ size: 7.2, opacity: .92 }});
        }}).join('')}}
        ${{points.map(({{ item, point }}) => {{
          ...
          const radius = isSelected ? 7.4 : (isFocused ? 7 : 3.6 + heat * 2.2);
          ...
          const showLabel = visibleLabels.has(item.name);
          return `<g class="industry-dot" data-name="${{item.name}}" data-focus-key="dot-${{item.rank}}" role="button" tabindex="0" aria-label="选择${{item.name}}" style="cursor:pointer">
            ...
            ${{showLabel ? `<text class="dot-label ${{isSelected || isFocused ? 'selected' : ''}}" x="${{labelX}}" y="${{labelY}}" fill="#17201c" opacity="${{isSelected || isFocused ? 1 : .82}}">${{item.name}}</text>` : ''}}
          </g>`;
        }}).join('')}}
      `;

...
<body>
  ...
  <script>
    ...
    function onIndustryKey(event, name) {{
    ...
    function renderRotation() {{
      ...
      svg.innerHTML = `
        ...
        ${{points.map(({{ item, point }}) => {{
          ...
          return `<g class="industry-dot" data-name="${{item.name}}" role="button" tabindex="0" aria-label="选择${{item.name}}" style="cursor:pointer">
      ...
      svg.querySelectorAll('.industry-dot').forEach(node => {{
        node.addEventListener('click', () => selectIndustry(node.dataset.name));
        node.addEventListener('keydown', event => onIndustryKey(event, node.dataset.name));
      }});
    }}
      ...
      document.getElementById('rankings').innerHTML = groups.map(([title, items]) => `
        <div class="rank-list">
          ...
          ${{items.map(name => {{
            ...
            return `<button class="rank-item" data-name="${{name}}" type="button" aria-label="选择${{name}}"><span>${{name}}</span><span class="score">${{fmt(item?.score)}}</span></button>`;
      ...
      document.getElementById('signal-groups').innerHTML = groups.map(([title, tone, rule, items, meta]) => `
        <div class="signal-card">
          ...
          ${{items.slice(0, 5).map(item => `
            <button class="signal-item" data-name="${{item.name}}" type="button" aria-label="选择${{item.name}}">
